...
class SearchMemoryRequest(BaseModel):
    ...
    project_path: Optional[str] = Field(default=None, description="可选的项目路径过滤")
...
class SyncOperation(BaseModel):
    ...
    memory_id: Optional[str] = Field(default=None, description="记忆ID，update/delete时需要")
    data: Optional[CreateMemoryRequest] = Field(default=None, description="创建/更新时的数据")
...
async def list_memories(
    project_path: Optional[str] = Query(default=None, description="项目路径过滤"),
    ...
    """列出用户记忆"""
    # 解析 memory_type
    parsed_type: Optional[MemoryType] = None
    if memory_type:
        try:
