...
async def handle_websocket(websocket: WebSocket, client_id: str) -> None:
    ...
    logger.info(
        f"[WS] Buffer created for {client_id}, buffer_id={id(buffer)}, container_buffers={len(container.state.client_buffers)}"
    )
        ...
        while True:
            ...
            if msg_type == "subscribe":
                ...
                if did and container.session_manager:
                    ...
                    if snap:
                        await buffer.send(
                            {"type": "dialog:snapshot", "data": snap, "timestamp": timestamp_ms()}
                        )
