"""DeepSeek Adapter - DeepSeek API 响应适配器
...
class DeepSeekAdapter(LLMResponseAdapter):
    ...
    DeepSeek API 与 OpenAI 兼容，但在 reasoning_content 字段中有独特之处。

    Example:
        >>> adapter = DeepSeekAdapter()
        >>> response = adapter.parse_response(raw_deepseek_response)
    ...
    def parse_response(self, raw_response: Any) -> UnifiedLLMResponse:
        ...
        DeepSeek 响应特点：
        - 与 OpenAI API 兼容的格式
        - 推理内容可能在 reasoning_content 字段
        - Token 用量在 usage 字段（prompt_tokens, completion_tokens）

        Args:
            raw_response: DeepSeek API 响应对象
        ...
        if hasattr(raw_response, "content"):
            ...
            model = self.detect_model(raw_response) or "deepseek-unknown"
        ...
        if isinstance(raw_response, dict):
            ...
            model = raw_response.get("model", "deepseek-unknown")
        ...
        if isinstance(raw_response, dict):
            ...
            if choices and len(choices) > 0:
                ...
                if message.get("reasoning_content"):
                    ...
                    metadata["reasoning_length"] = len(message["reasoning_content"])
