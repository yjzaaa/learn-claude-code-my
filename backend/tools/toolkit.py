...
提供 @tool 装饰器和工具构建函数。
从原始 agents/base/toolkit.py 迁移而来。
"""
...
def _build_parameters_from_signature(func: Callable[..., Any]) -> JSONSchema:
...
def build_tools(functions: list[Callable[..., Any]]) -> list[MergedToolItem]:
    """从工具函数列表构建合并后的 tools（每项内含 handler，OpenAI 格式）。"""
    tools: list[MergedToolItem] = []
    names: set[str] = set()
...
def scan_tools(namespace: dict[str, Any] | types.ModuleType) -> list[MergedToolItem]:
