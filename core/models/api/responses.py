...
from core.models.base import Response
from core.models.mixins import DialogRefMixin
from core.models.websocket_models import WSDialogSnapshot
...
class ResultModel(Response):
    ...
    @classmethod
    def ok(cls, message: str = "Success", data: Optional[Any] = None, **kwargs) -> "ResultModel":
        """创建成功结果"""
        return cls(success=True, message=message, data=data, **kwargs)

    @classmethod
    def error(cls, message: str = "Error", data: Optional[Any] = None, **kwargs) -> "ResultModel":
        """创建错误结果"""
...
class CreateDialogResponse(Response):
    """创建对话响应模型"""
    data: Optional[WSDialogSnapshot]
