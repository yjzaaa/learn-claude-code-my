...
def run_sql_query(sql: str, limit: int = 200, inspect_structure: bool = False) -> str:
    """Execute SQL against configured SQL Server via pyodbc.

    If `inspect_structure` is True, the function will query the database schema
    and return table structures and sample data.
    """
    try:
        import pyodbc
    ...
    try:
        ...
        if inspect_structure:
            ...
            for table in tables:
                ...
                structure[table] = {col[0]: col[1] for col in columns}
            ...
            return json.dumps({"structure": structure, "samples": samples}, ensure_ascii=False)
