...
class SkillLoader:
    def __init__(self, skills_dir: Path) -> None:
        ...
        self.backups_dir: Path = WORKDIR / ".backups" / "skills"
        self._load_all()
    ...
    def update_skill(self, name: str, old_text: str | None = None, new_text: str | None = None,
                     full_content: str | None = None, reason: str = "") -> str:
        """更新 skill 文件，自动备份。
    ...
    def _reload_skill(self, name: str) -> None:
        """重新加载单个 skill。"""
        skill = self.skills.get(name)
        if not skill:
            return
...
# 技能加载工具放前面，让模型优先看到
_base_tools = OPS.get_tools()
if isinstance(_base_tools, dict):
    _base_tools = list(_base_tools.values())
TOOLS: list[Callable[..., Any]] = [load_skill, load_skill_reference, load_skill_script, update_skill] + _base_tools
...
def _write_latest_log(messages: list[dict[str, Any]]) -> None:
    """覆盖旧日志，仅保留最新的完整会话快照。"""
...
def _on_stop(messages: list[dict[str, Any]], response: Any) -> None:
    """在模型结束本轮对话时写入最新日志快照。"""
...
def _on_stream_token(token: str, block: Any, messages: list[dict[str, Any]], response: Any) -> None:
    """按 token 粒度流式打印模型输出。"""
    _ = block
    _ = messages
    _ = response
