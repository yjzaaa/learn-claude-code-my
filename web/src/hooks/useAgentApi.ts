...
export function useAgentApi() {
  ...
  const createDialog = useCallback(async (title: string = "New Dialog"): Promise<ApiResponse<Dialog>> => {
    return request("/api/dialogs", {
      method: "POST",
  ...
  const getDialog = useCallback(async (dialogId: string): Promise<ApiResponse<Dialog>> => {
    return request(`/api/dialogs/${dialogId}`);
  }, [request]);

  const deleteDialog = useCallback(async (dialogId: string): Promise<ApiResponse<void>> => {
    return request(`/api/dialogs/${dialogId}`, {
      method: "DELETE",
    });
  }, [request]);

  const getMessages = useCallback(async (dialogId: string): Promise<ApiResponse<any[]>> => {
  ...
  const sendMessage = useCallback(async (
    ...
    const result = await request(`/api/dialogs/${dialogId}/messages`, {
      method: "POST",
  ...
  const getSkill = useCallback(async (skillName: string): Promise<ApiResponse<{ name: string; content: string }>> => {
    return request(`/api/skills/${skillName}`);
  }, [request]);

  const loadSkill = useCallback(async (skillName: string): Promise<ApiResponse<{ name: string; content: string }>> => {
  ...
  const getAgentStatus = useCallback(async (): Promise<ApiResponse<{
    is_running: boolean;
